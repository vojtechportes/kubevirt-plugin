...
Back to Compute resources and Next again, and are reflected on the Review and create step.
...
- **In-Scope:** Custom configuration flow through Guest OS (async OS type load gating Next), Boot
  ...
  modal and editing it twice; opening the Boot order modal and reordering devices twice; Back/Next
  round-trip persistence of both edits; Review step display of the generated VM name and visible
  sections; VM creation.
...
- **Configuration:** A boot volume should be available; if none exist, the wizard falls back to "No
  boot source" and the test continues.
...
- **Objective:** Verify that editing the Description and Boot order fields twice on the
  ...
  Next round trip, and that the Review step displays the generated VM name and all expected
  sections before the VM is created.
...
| 3    | Select the first available boot volume (or "No boot source"); click Next                        | Boot source step is visible; wizard advances to Compute resources                                                   |
...
| 6    | Open the Boot order modal, reorder a device, save; reopen, reorder again, save                  | Each save renders the same number of devices in the updated order on the Details tab                                |
