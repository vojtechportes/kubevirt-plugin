...
- **In-Scope:** Reopening the wizard after Cancel and verifying the creation method reverts to
  ...
  successful VM creation and verifying the same reset, plus that the Guest OS step's default OS
  type selection is restored rather than carrying over any previous selection.
...
| Step | Action                                                                                                       | Expected Result                                                                         |
| :--- | :----------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------- |
| 1    | Complete the Custom configuration wizard (generate name, no boot source, edit Description) and create the VM | The console redirects to VM details; the VM exists via API                              |
...
| 3    | Generate a new VM name and click Next                                                                        | The Guest OS step shows a non-empty default selection, not the previous session's state |
