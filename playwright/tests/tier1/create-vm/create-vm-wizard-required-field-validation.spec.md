...
- **Configuration:** At least one template and one existing VirtualMachine must be available for the
  Template and Clone checks respectively.
- **Initial Setup:** Each test opens the wizard from the shared job test namespace
  (`testConfig.testNamespace`); no VM is created or resource tracked, since these tests only assert
  Next-button state and cancel the wizard afterward.
...
- **Target version:** CNV 5.1.0
- **Jira References:** NO-JIRA-0
- **Pre-conditions:** At least one existing VirtualMachine is available to clone.
- **Tags:** `@adminOnly`

| Step | Action                                               | Expected Result                                                   |
| :--- | :--------------------------------------------------- | :---------------------------------------------------------------- |
| 1    | Select Clone existing VirtualMachine                 | Clone is selected; Next is already enabled — Name is not required |
| 2    | Click Next; verify the Source step is visible        | Source step is visible                                            |
| 3    | Observe the Next button before selecting a source VM | Next is disabled                                                  |
| 4    | Select the first available source VM                 | Next becomes enabled                                              |
...
| NO-JIRA-0   | `003`        | Functional smoke | Automated |
