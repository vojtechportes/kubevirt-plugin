...
test.describe(
  ...
  () => {
    ...
    test('Create from Template — Name is required, then a Template selection is required', async ({
      ...
      expect(nextDisabledBeforeTemplate, 'Next should be disabled before selecting a template').toBe(
        true,
      );
      ...
      expect(
        nextDisabledAfterTemplate,
        'Next should be enabled once a template is selected',
      ).toBe(false);
    ...
    test('Clone existing VirtualMachine — Name is not required, but a source VM selection is', async ({
      ...
      expect(
        nextDisabledBeforeSource,
        'Next should be disabled before selecting a source VM',
      ).toBe(true);
      ...
      expect(
        nextDisabledAfterSource,
        'Next should be enabled once a source VM is selected',
      ).toBe(false);
