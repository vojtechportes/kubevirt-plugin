...
- **Objective:** Verify that selecting the Memory Intensive (M) instance type series and the
  largest compute size updates the size dropdown text, and that the resulting VM references an
  instance type from that series via the API.
- **Target version:** CNV 5.1.0
- **Jira References:** NO-JIRA-0
- **Pre-conditions:** None
- **Tags:** `@adminOnly`
...
| 2    | Select the Memory Intensive (M) series and the largest compute size      | Size dropdown text contains "CPUs"                    |
