...
// Processing only requires a value (or a generator). Main's editor separately
// enforces the five-character minimum on every required parameter except NAME.
export const templateParameterPresenceSchema = yup
  ...
  .test(
    ...
    (parameter) =>
      !parameter.required ||
      parameter.name === NAME_INPUT_FIELD ||
      Boolean(parameter.value?.trim()) ||
      Boolean(parameter.generate),
  );
...
// Selection is valid before parameter editing; processing uses the presence
// schema above and the editor uses createTemplateParametersSchema.
export const createTemplateSchema = (t: TFunction): yup.ObjectSchema<VMWizardTemplateValues> =>
  yup
    .object({
      generationRevision: yup.number().defined(),
