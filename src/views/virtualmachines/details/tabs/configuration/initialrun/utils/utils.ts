...
export type SubmitSysprepVM = (updatedVM: V1VirtualMachine) => V1VirtualMachine | undefined;
...
export const createSysprepConfigMap = async (
  ...
  const disks = [...(vmDisks ?? []), sysprepDisk()];
  const volumes = [...(vmVolumes ?? []), sysprepVolume(configMap.metadata.name)];
